...
# ╔═╡ e1d7a0d1-6c5e-4570-9120-b89504733a4c
html"""
<script>
	(function() {
		const loc = window.location.search;
		const sessionId = new URLSearchParams(loc).get('id');

		let exportCard = document.getElementById('statefileExport');
		if(exportCard) {
			exportCard.remove();
		}

		exportCard = document.createElement('a');
		exportCard.setAttribute('href', `/statefile?id=${sessionId}`);
		exportCard.setAttribute('id', 'statefileExport')
		exportCard.setAttribute('class', 'export_card');

		const cardHeader = document.createElement('header');
		cardHeader.innerText = '✨ State File';
		exportCard.appendChild(cardHeader);
	
		const cardDescription = document.createElement('section');
		cardDescription.innerText = 'A .plutostate file for straightforward publication.';
		exportCard.appendChild(cardDescription);

		const exportContainer = document.getElementById('container');
		exportContainer.insertBefore(exportCard, exportContainer.children[exportContainer.children.length-1]);
	})();
</script>
"""
